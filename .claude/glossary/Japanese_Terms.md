...
| Drifter/Maiden/Reaver/Mystica/Anae/Rifter（`[Forgotten]`系モンスター種族名、`Movers.csv`の`PROPMOVER_TXT_505001〜505030`準拠） | ドリフター／メイデン／リーバー／ミスティカ／アナエ／リフター（接頭語`[Forgotten]`は「[忘れられし]」） | `Items.csv`の`PROPITEM_TXT_913758〜913769`（Spirit Shard系アイテムの説明文で種族名のみ再利用、`[Forgotten]`の接頭語は付けない）で確認（2026-07-29）。`Mystica`は`Achievements.csv`の`Okean`とは別モンスター（Okeanは「オケアノス」）。`[Forgotten]`接頭語は当初「[忘却の]」だったが2026-08-12にユーザー指示で「[忘れられし]」に変更（`Movers.csv`の`PROPMOVER_TXT_505001〜505030`全30件を修正済み）。`Items.csv`の`Ghost of the Forgotten ~`系（`PROPITEM_TXT_827778〜827790`＝「忘却の幽霊〜」）は別シリーズのためこの変更の対象外 |
